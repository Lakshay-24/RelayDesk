...
export async function POST(request:Request){
 ...
 const {data:conversation}=await supabase
  .from("conversations")
  .select("id,workspace_id,channel,subject,contact_id,contacts(email),workspaces(name,slug),messages(external_message_id,created_at)")
 ...
 if(conversation.channel==="email"){
  ...
  const messageRows=((conversation.messages??[]) as {external_message_id:string|null;created_at:string}[])
   .filter(item=>Boolean(item.external_message_id))
   .sort((a,b)=>+new Date(a.created_at)-+new Date(b.created_at));
  const references=messageRows.map(item=>item.external_message_id as string);
