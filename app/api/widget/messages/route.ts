...
async function verify(db: ReturnType<typeof createAdminClient>, workspaceKey: string, visitorKey: string, conversationId: string) {
  const { data } = await db
    ...
    .eq("workspaces.public_key", workspaceKey)
    .eq("contacts.visitor_key", visitorKey)
    .maybeSingle();
  return data;
}
...
export async function POST(request: Request) {
  ...
  const { data: message, error } = await db
    ...
    .select("id,conversation_id,sender_type,body,read_at,agent_read_at,visitor_read_at,created_at")
    .single();
...
export async function GET(request: Request) {
  ...
  const parsed = z.object({
    ...
    visitorKey: url.searchParams.get("visitorKey"),
