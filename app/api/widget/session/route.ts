...
const schema = z.object({ workspaceKey: z.string().uuid(), visitorKey: z.string().uuid(), name: z.string().trim().max(100).optional(), email: z.string().email().optional() });
export async function POST(request: Request) {
  ...
  if (!contact) {
    ...
    contact = result.data;
  } else await db.from("contacts").update({ last_seen_at: new Date().toISOString() }).eq("id", contact.id);
  ...
  const { data: messages } = await db.from("messages").select("id,conversation_id,sender_type,body,read_at,created_at").eq("conversation_id", conversation.id).order("created_at");
  return NextResponse.json({ workspace: { name: workspace.name }, conversationId: conversation.id, messages: messages ?? [] });
}
