...
async function memberContext() {
  ...
  const { data: membership } = await db.from("memberships").select("id,workspace_id,role").eq("user_id", user.id).limit(1).maybeSingle();
  if (!membership) return { error: Response.json({ error: "Workspace access required" }, { status: 403 }) } as const;
