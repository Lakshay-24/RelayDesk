...
const createSchema = z.object({
  ...
  hostname: z.string().trim().min(4).max(253).regex(/^(?=.{1,253}$)(?!-)(?:[a-z0-9-]{1,63}\.)+[a-z]{2,63}$/i),
});
const domainSchema = z.object({ domainId: z.string().uuid(), workspaceId: z.string().uuid() });

async function requireAdmin(db: Awaited<ReturnType<typeof createClient>>, workspaceId: string, userId: string) {
  return db.from("memberships").select("id").eq("workspace_id", workspaceId).eq("user_id", userId).eq("role", "admin").maybeSingle();
}

function vercelConfig() {
  const token = process.env.VERCEL_TOKEN;
  const project = process.env.VERCEL_PROJECT_ID || process.env.VERCEL_PROJECT_NAME || "relay-desk-mjq6";
  const teamId = process.env.VERCEL_TEAM_ID;
  return token ? { token, project, teamId } : null;
}

function projectDomainUrl(project: string, hostname?: string, action?: "verify") {
  const encodedProject = encodeURIComponent(project);
  const encodedDomain = hostname ? `/${encodeURIComponent(hostname)}` : "";
  const suffix = action === "verify" ? "/verify" : "";
  const team = process.env.VERCEL_TEAM_ID ? `?teamId=${encodeURIComponent(process.env.VERCEL_TEAM_ID)}` : "";
  return `https://api.vercel.com/v9/projects/${encodedProject}/domains${encodedDomain}${suffix}${team}`;
}

async function vercelRequest(method: "POST" | "DELETE", hostname: string, action?: "verify") {
  const config = vercelConfig();
  if (!config) return { automated: false as const, ok: false as const, message: "Owner-side Vercel automation is not configured." };
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Enter a subdomain such as help.company.com." }, { status: 400 });

  const db = await createClient();
  const { data: { user } } = await db.auth.getUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const { data: membership } = await requireAdmin(db, parsed.data.workspaceId, user.id);
