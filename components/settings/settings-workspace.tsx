...
export function SettingsWorkspace({workspace,membership,initialDomains,inboundAddress,memberCount}:Props){
  ...
  async function loadTeam(){
    try{
      ...
      setMembers(json.members??[]);setInvitations(json.invitations??[]);
  ...
  function invite(){void run("invite",async()=>{
    ...
    setEmail("");await loadTeam();setNotice({tone:"success",text:json.warning??"Invitation sent"});
  });}
  ...
  function addDomain(){void run("domain-new",async()=>{
    ...
    setDomains(current=>[...current,json.domain]);setHostname("");setNotice({tone:"success",text:"Domain added. Create the TXT and CNAME records shown below."});
  });}
  ...
  function verify(domain:Domain){void run(`domain-${domain.id}`,async()=>{
    try{
      ...
      setDomains(current=>current.map(item=>item.id===domain.id?json.domain:item));setNotice({tone:"success",text:"Domain ownership verified. Complete Vercel registration and SSL provisioning next."});
  ...
  function removeDomain(domain:Domain){
    ...
    void run(`domain-${domain.id}`,async()=>{
      ...
      setDomains(current=>current.filter(item=>item.id!==domain.id));setNotice({tone:"success",text:"Domain removed from RelayDesk."});
    });
  }
  ...
  return <section className="settings-page">
    ...
    <div className="settings-grid">
      <div className="section-card"><h3><Copy size={16}/>Install messenger</h3><p className="muted">Paste this before the closing body tag on any site.</p><pre className="code-block">{snippet}</pre><button className="chip" onClick={async()=>{await navigator.clipboard.writeText(snippet);setNotice({tone:"success",text:"Messenger snippet copied"});}}>Copy snippet</button></div>
      ...
      <div className="section-card"><h3><UserPlus size={16}/>Invite teammate</h3><label>Email<input type="email" value={email} disabled={busy("invite")} onChange={event=>setEmail(event.target.value)} placeholder="agent@company.com"/></label><label>Role<select value={inviteRole} disabled={busy("invite")} onChange={event=>setInviteRole(event.target.value as "admin"|"agent")}><option value="agent">Agent</option><option value="admin">Admin</option></select></label><button className="primary-button" aria-busy={busy("invite")} disabled={Boolean(action)||!email.trim()} onClick={invite}>{busy("invite")?"Sending invite…":"Send invite"}</button>{invitations.length>0&&<div><h4>Pending invitations</h4>{invitations.map(item=><p key={item.id} className="muted">{item.email} · {item.role} · expires {new Date(item.expires_at).toLocaleDateString()}</p>)}</div>}</div>
      <div className="section-card"><h3><Users size={16}/>Team</h3>{members.map(member=><div key={member.id} className="team-row"><div><strong>{member.name||member.email||"Team member"}</strong><p className="muted">{member.email||"No email"}</p></div><select disabled={!isAdmin||Boolean(action)} value={member.role} onChange={event=>changeMember(member.id,event.target.value as "admin"|"agent")}><option value="admin">Admin</option><option value="agent">Agent</option></select>{isAdmin&&member.id!==membership.id&&<button className="chip" aria-busy={busy(`member-${member.id}`)} disabled={Boolean(action)} onClick={()=>removeMember(member.id)}><Trash2 size={14}/>{busy(`member-${member.id}`)?"Removing…":"Remove"}</button>}</div>)}{!members.length&&<p className="muted">Loading team…</p>}</div>
      <CannedResponsesManager/>
      <WebhooksManager isAdmin={isAdmin}/>
      <ApiAccessManager isAdmin={isAdmin}/>
      <div className="section-card"><h3><Globe2 size={16}/>Custom help domain</h3><label>Hostname<input value={hostname} disabled={busy("domain-new")} onChange={event=>setHostname(event.target.value.toLowerCase().trim())} placeholder="help.company.com"/></label><button className="primary-button" aria-busy={busy("domain-new")} disabled={!isAdmin||Boolean(action)||!hostname.trim()} onClick={addDomain}>{busy("domain-new")?"Adding domain…":"Add domain"}</button>{domains.map(domain=><div key={domain.id} className="section-card"><strong>{domain.hostname}</strong><p className="muted">Status: {domain.status}</p><p className="muted">TXT _relaydesk.{domain.hostname} = {domain.verification_token}</p><p className="muted">CNAME {domain.hostname} = {process.env.NEXT_PUBLIC_CUSTOM_DOMAIN_CNAME_TARGET||"cname.vercel-dns.com"}</p><div className="filters"><button className="chip" aria-busy={busy(`domain-${domain.id}`)} disabled={Boolean(action)||domain.status==="verified"} onClick={()=>verify(domain)}>{domain.status==="verified"?<><Check size={14}/>Verified</>:busy(`domain-${domain.id}`)?"Checking DNS…":domain.status==="failed"?"Retry DNS":"Verify DNS"}</button><button className="chip danger-chip" disabled={Boolean(action)||!isAdmin} onClick={()=>removeDomain(domain)}><Trash2 size={14}/>{busy(`domain-${domain.id}`)?"Removing…":"Remove"}</button></div></div>)}</div>
    </div>
  </section>;
}
