...
export function SettingsWorkspace({
  ...
  async function loadTeam() {
    ...
    if (response.ok) {
      ...
      setNotice(json.error ?? "Could not load team");
    }
  }
  ...
  async function invite() {
    ...
    const json = await response.json();
    if (response.ok) {
      setNotice(json.warning ?? "Invitation sent");
      setEmail("");
      await loadTeam();
    } else {
      setNotice(json.error ?? "Could not invite");
    }
    setTeamBusy(false);
  }
  ...
  async function changeMember(membershipId: string, role: "admin" | "agent") {
    ...
    if (response.ok) {
      ...
      setNotice(json.error ?? "Could not update member");
    }
    setTeamBusy(false);
  }
  ...
  async function removeMember(membershipId: string) {
    ...
    if (response.ok) {
      ...
      setNotice(json.error ?? "Could not remove member");
    }
    setTeamBusy(false);
  }
  ...
  async function addDomain() {
    ...
    if (response.ok) {
      ...
      setNotice(json.error ?? "Could not add domain");
    }
  }
  ...
  async function verify(domain: Domain) {
    ...
    if (response.ok) {
      ...
      setNotice(json.error ?? "Could not verify domain");
    }
  }
    ...
    <section className="settings-page">
      <div className="page-title-row">
        ...
        <span className="muted">
          {membership.role} · {visibleMemberCount} member{visibleMemberCount === 1 ? "" : "s"}
        </span>
      </div>
      ...
      <div className="settings-grid">
        <div className="section-card">
          <h3><Copy size={16} /> Install messenger</h3>
          <p className="muted">Paste this before the closing body tag on any site.</p>
          <pre className="code-block">{snippet}</pre>
          <button className="chip" onClick={() => navigator.clipboard.writeText(snippet)}>Copy snippet</button>
        </div>
        ...
        <div className="section-card">
          ...
          <p className="muted">Inbound delivery requires the configured email provider webhook.</p>
        </div>
        ...
        <div className="section-card">
          ...
          <button className="primary-button" disabled={teamBusy || !email.trim()} onClick={invite}>Send invite</button>
          {invitations.length > 0 && (
            <div>
              <h4>Pending invitations</h4>
              {invitations.map((item) => (
                <p key={item.id} className="muted">
                  {item.email} · {item.role} · expires {new Date(item.expires_at).toLocaleDateString()}
                </p>
              ))}
            </div>
          )}
        ...
        <div className="section-card">
          ...
          {members.map((member) => (
            <div key={member.id} className="team-row">
              <div>
                <strong>{member.name || member.email || "Team member"}</strong>
                <p className="muted">{member.email || "No email"}</p>
              </div>
              <select
                disabled={membership.role !== "admin" || teamBusy}
                value={member.role}
                onChange={(event) => void changeMember(member.id, event.target.value as "admin" | "agent")}
              >
                <option value="admin">Admin</option>
                <option value="agent">Agent</option>
              </select>
              {membership.role === "admin" && member.id !== membership.id && (
                <button className="chip" disabled={teamBusy} onClick={() => void removeMember(member.id)}>
        ...
        <div className="section-card">
          ...
          <label>
            Hostname
            <input value={hostname} onChange={(event) => setHostname(event.target.value)} placeholder="help.company.com" />
          </label>
          <button className="primary-button" onClick={addDomain}>Add domain</button>
          {domains.map((domain) => (
            <div key={domain.id} className="section-card">
              <strong>{domain.hostname}</strong>
              <p className="muted">TXT _relaydesk.{domain.hostname} = {domain.verification_token}</p>
              <button className="chip" onClick={() => verify(domain)}>
                {domain.status === "verified" ? <><Check size={14} /> Verified</> : "Verify DNS"}
              </button>
            </div>
          ))}
        </div>
